...
if (isset($_GET['feature_id'])) {
    ...
    $sql = "SELECT image FROM feture WHERE id = :id";
    ...
    if ($slider) {
        ...
        $deleteSql = "DELETE FROM feture WHERE id = :id";
        $delStmt = $db->dbHandler->prepare($deleteSql);
        $delStmt->bindParam(':id', $id);
        $delStmt->execute();

        $_SESSION['msg'] = "Feture deleted successfully!";
    } else {
        $_SESSION['msg'] = "Feture not found!";
    }
}
...
<div class="inner-wrapper">
    ...
    <section role="main" class="content-body content-body-modern mt-5">
        <header class="page-header page-header-left-inline-breadcrumb">
            <h2 class="font-weight-bold text-6">Feature List </h2>
                            ...
                            <div class="datatable-header">
                                <div class="row align-items-center mb-3">
                                    <div class="col-12 col-lg-auto mb-3 mb-lg-0">
                                        <a href="/shop/admin/fetureCreate.php" class="btn btn-primary btn-md font-weight-semibold btn-py-2 px-4">+ Add Feature</a>
                                    </div>
                                    ...
                                    <?php if (!empty($data)) : ?>
                                        <?php foreach ($data as $item) : ?>
                                            <tr>
                                                ...
                                                <td><?= $item['id'] ?></td>
                                                <td><img src="<?= $item['image'] ?>" alt="Slider Image" width="50"></td>
                                                <td><?= $item['subtitle'] ?></td>
                                                <td><?= $item['isActive'] == 1 ? 'Active' : 'Deactive' ?></td>
                                                <td><?= $item['create_at'] ?? '' ?></td>
                                                <td>
                                                    <a href="./fetureEdit.php?feature_id=<?= $item['id'] ?>" class="btn btn-primary">Edit</a>
                                                    <a href="fetureIndex.php?feature_id=<?= $item['id']; ?>"
